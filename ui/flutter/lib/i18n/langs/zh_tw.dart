const zhTW = {
  "zh_TW": {
    'label': '中文 (正體)',
    'error': '錯誤',
    'tip': '提示',
    'confirm': '確認',
    'confirmDelete': '確定要刪除嗎？',
    'cancel': '取消',
    'on': '開啟',
    'off': '關閉',
    'selectAll': '全選',
    'select': '選擇',
    'task': '任務',
    'downloading': '下載中',
    'downloaded': '已下載',
    'setting': '設定',
    'donate': '捐款',
    'exit': '退出',
    'create': '建立任務',
    'directDownload': '直接下載',
    'advancedOptions': '進階選項',
    'followSettings': '跟隨設定',
    'downloadLink': '下載連結',
    'downloadLinkValid': '請輸入下載連結',
    'downloadLinkHit': '請輸入下載連結，支援批量下載，每行一個連結@append',
    'downloadLinkHitDesktop': '，或直接拖曳種子檔到此處',
    'download': '下載',
    'noFileSelected': '請至少選擇一個檔案以繼續。',
    'noStoragePermission': '需要儲存空間權限',
    'selectFile': '選擇檔案',
    'rename': '重新命名',
    'basic': '基本',
    'advanced': '進階',
    'general': '一般',
    'downloadDir': '下載目錄',
    'downloadDirValid': '請選擇下載目錄',
    'connections': '連接數',
    'useServerCtime': '使用伺服器時間作為檔案建立時間',
    'maxRunning': '最大執行任務數',
    'defaultDirectDownload': '預設勾選直接下載',
    'autoStartTasks': '啟動時自動開始未完成的任務',
    'autoTorrentEnable': '自動從 .torrent 檔案建立 BT 任務',
    'autoTorrentDeleteAfterDownload': '建立 BT 任務後刪除 .torrent 檔案',
    'autoDeleteMissingFileTasks': '自動刪除缺失檔案任務',
    'items': '@count 個項目',
    'subscribeTracker': '訂閱 Tracker',
    'subscribeFail': '訂閱失敗，請檢查網路或稍後再試',
    'update': '更新',
    'updateDaily': '每日更新',
    'lastUpdate': '最後更新時間：@time',
    'addTracker': '新增 Tracker',
    'addTrackerHit': '請輸入 Tracker 伺服器 URL，每行一個',
    'ui': '介面',
    'theme': '主題',
    'themeSystem': '系統',
    'themeLight': '淺色',
    'themeDark': '深色',
    'locale': '語言',
    'about': '關於',
    'homepage': '首頁',
    'version': '版本',
    'protocol': '協議',
    'port': '連接埠',
    'apiToken': 'API Token',
    'notSet': '未設定',
    'set': '已設定',
    'portInUse': '連接埠 [@port] 已被使用，請更改連接埠',
    'effectAfterRestart': '重新啟動後生效',
    'developer': '開發者',
    'logDirectory': '日誌目錄',
    'show': '顯示',
    'continue': '繼續',
    'pause': '暫停',
    'startAll': '全部開始',
    'pauseAll': '全部暫停',
    'deleteTask': '刪除 @count 個任務',
    'deleteTaskTip': '保留已下載的檔案',
    'delete': '刪除',
    'newVersionTitle': '發現新版本 @version',
    'newVersionUpdate': '立即更新',
    'newVersionLater': '稍後',
    'extensions': '擴充功能',
    'extensionInstallUrl': '安裝 URL',
    'extensionInstallSuccess': '安裝成功',
    'extensionUpdateSuccess': '更新成功',
    'extensionDelete': '刪除擴充功能',
    'extensionAlreadyLatest': '已是最新版本',
    'extensionFind': '尋找擴充功能',
    'extensionDevelop': '開發擴充功能',
    'history': '歷史記錄',
    'clearHistory': '清除歷史記錄',
    'noHistoryFound': '未找到歷史記錄',
    'serviceTitle': '下載服務',
    'serviceText': '運行中',
    'network': '網路',
    'proxy': 'Proxy',
    'noProxy': '沒有 Proxy',
    'systemProxy': '系統 Proxy',
    'customProxy': '自訂 Proxy',
    'server': '伺服器',
    'username': '使用者名稱',
    'password': '密碼',
    'thanks': '感謝',
    'thanksDesc': '感謝所有幫助建立和發展 Gopeed 社群的貢獻者！',
    'browserExtension': '瀏覽器擴充功能',
    'launchAtStartup': '開機自動執行',
    'runAsMenubarApp': '以選單列應用程式執行',
    'runAsMenubarAppDesc': '隱藏 Dock 圖示，僅在選單列執行',
    'seedConfig': '做種設定',
    'seedKeep': '持續做種直到手動停止',
    'seedRatio': '做種分享率',
    'seedTime': '做種時間（分鐘）',
    'setAsDefaultBtClient': '設為系統預設 BT 客戶端',
    'taskDetail': '任務詳情',
    'taskName': '任務名稱',
    'taskUrl': '任務連結',
    'downloadPath': '下載路徑',
    'skipVerifyCert': '跳過憑證驗證',
    'archives': '壓縮檔',
    'autoExtract': '自動解壓縮壓縮包',
    'archivePassword': '壓縮包密碼',
    'archivePasswordHint': '如無密碼請留空',
    'deleteAfterExtract': '解壓後刪除壓縮檔',
    'extracting': '正在解壓',
    'extractDone': '解壓完成',
    'extractError': '解壓失敗',
    'waitingParts': '等待分卷',
    'name': '名稱',
    'size': '大小',
    'unknown': '未知',
    'fileSelectedCount': '文件數：',
    'fileSelectedSize': '大小：',
    'httpHeaderName': '標頭名稱',
    'httpHeaderValue': '標頭值',
    'insertPlaceholder': '插入佔位符',
    'placeholderYear': '當前年份',
    'placeholderMonth': '當前月份 (01-12)',
    'placeholderDay': '當前日期 (01-31)',
    'placeholderDate': '完整日期 (年-月-日)',
    'example': '例如 @value',
    'downloadCategories': '下載目錄分類',
    'categoryMusic': '音樂',
    'categoryVideo': '影片',
    'categoryDocument': '文件',
    'categoryProgram': '程式',
    'categoryName': '分類名稱',
    'categoryPath': '分類路徑',
    'builtInCategory': '內建分類無法刪除',
    'selectCategory': '選擇分類',
    'githubMirror': 'GitHub 鏡像',
    'githubMirrorEnable': '啟用 GitHub 鏡像',
    'githubMirrorDesc': '使用鏡像站點加速 GitHub 內容下載',
    'githubMirrorType': '鏡像類型',
    'githubMirrorUrl': '鏡像地址',
    'githubMirrorUrlHint': '請輸入鏡像地址',
    'notifyWhenNewVersion': '有新版本時提醒',
    'analyticsEnabled': '上傳統計數據',
    'analyticsEnabledDesc': '分享匿名使用數據以幫助我們改進',
    'webhook': 'Webhook 推送',
    'webhookEnable': '啟用 Webhook',
    'webhookDesc': '在任務完成或失敗時發送 HTTP POST 通知',
    'webhookUrlHint': '請輸入 Webhook URL',
    'webhookTest': '測試',
    'webhookTestSuccess': 'Webhook 測試成功',
    'webhookTestFail': 'Webhook 測試失敗',
    'urlInvalid': '請輸入有效的 HTTP 或 HTTPS 鏈接',
    'required': '此項為必填項',
    'add': '新增',
    'edit': '編輯',
    'login': '登入',
    'username_required': '請輸入用戶名',
    'password_required': '請輸入密碼',
    'login_success': '登入成功',
    'login_failed': '登入失敗，請檢查用戶名和密碼',
    'login_failed_network': '登入失敗，請檢查網路連線',
    'notifyDownloadComplete': '下載完成時通知',
    'downloadCompleteTitle': '下載完成',
    'downloadCompleteBody': '檔案 "@fileName" (@fileSize) 已成功下載',
    'downloadFailedTitle': '下載失敗',
    'downloadFailedBody': '檔案 "@fileName" 下載失敗: @error',
  }
};
